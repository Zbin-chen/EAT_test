...
from .NewCVC import GobleAttention
from .NewCVC import LocalAttention
from .NewCVC import BasicConv2d
...
class BasicDCN_V4(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, act=nn.ReLU(inplace=True), offset_scale=2.0):
        super(BasicDCN_V4, self).__init__()
        self.conv = DCNv4(in_planes, 3, 1, 1, group=in_planes // out_planes, offset_scale=offset_scale, bias=False,
                          layer_scale=1.0, dw_kernel_size=3, mlp_ratio=4., drop_path_rate=0.4, norm_layer='LN')
...
class GobleAttentionV4(nn.Module):
    def __init__(self, in_dim=1, out_dim=32, kernel_size=3, mlp_ratio=4, act=nn.GELU(), offset_scale=2.5):
        super(GobleAttentionV4, self).__init__()
        # 调整通道Conv
        # self.conv = nn.Conv2d(in_dim, out_dim, 3, 1, 1)
        # self.conv = DCNv4(in_dim, 3, 1, 1) v3.11
        # self.conv = DCNv4(in_dim, 3, 1, 1, group=in_dim//out_dim)
        self.conv = DCNv4(in_dim, 3, 1, 1, group=in_dim // out_dim, offset_scale=offset_scale, bias=False,
                          layer_scale=1.0, dw_kernel_size=3, mlp_ratio=4., drop_path_rate=0.4, norm_layer='LN')
        # self.conv1 = DCNv4(in_dim//2, 3, 1, 1)
        # self.conv2 = DCNv4(in_dim//2, 3, 1, 1)
        # 特征Norm
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        x = x.reshape(shape)
        x = torch.chunk(x, shape[1] // 32, dim=1)
        x_ = []
        for tt in x:
            x_.append(self.sep(tt))
...
class LocalAttentionV4(nn.Module):
    def __init__(self, in_dim=32, out_dim=32, offset_scale=1.0):
        super(LocalAttentionV4, self).__init__()
...
class AttentionBlockV4(nn.Module):
    def __init__(self, in_dim=3, out_dim=32, kernel_size=3, mlp_ratio=4, shallow=True, gla_v4=[True, True], gla_os=[2.5, 1.0]):
        super(AttentionBlockV4, self).__init__()
        self.in_dim = in_dim
        ...
        if shallow == True:
            ...
            self.act = Swish()
        if gla_v4[0] == True:
            self.gobel_attention = GobleAttentionV4(in_dim=in_dim // 2, out_dim=out_dim, kernel_size=kernel_size,
                                                    mlp_ratio=mlp_ratio, act=self.act, offset_scale=gla_os[0])
        else:
            self.gobel_attention = GobleAttention(in_dim=in_dim // 2, out_dim=out_dim, kernel_size=kernel_size,
                                                  mlp_ratio=mlp_ratio, act=self.act)
        if gla_v4[1] == True:
            self.local_attention = LocalAttentionV4(in_dim=in_dim // 2, out_dim=out_dim, offset_scale=gla_os[1])
        else:
...
class CVC_UnetrV4(nn.Module):
    def __init__(self, in_channels=3,
                 ...
                 L_feature_v4=True,
                 L_feature_os=2.0,
                 model_dir='/workspace/Encs/src/CVCUNETR/pvt_v2_b2.pth'):
        super(CVC_UnetrV4, self).__init__()
        ...
        self.block2 = AttentionBlockV4(in_dim=c2_in_channels, out_dim=out_dim, kernel_size=kernel_size,
                                       mlp_ratio=mlp_ratio, shallow=True, gla_v4=gla_v4, gla_os=gla_os)
        self.block3 = AttentionBlockV4(in_dim=c3_in_channels, out_dim=out_dim, kernel_size=kernel_size,
                                       mlp_ratio=mlp_ratio, shallow=False, gla_v4=gla_v4, gla_os=gla_os)
        self.block4 = AttentionBlockV4(in_dim=c4_in_channels, out_dim=out_dim, kernel_size=kernel_size,
                                       mlp_ratio=mlp_ratio, shallow=False, gla_v4=gla_v4, gla_os=gla_os)

        self.fuse2 = nn.Sequential(BasicConv2d(out_dim * 2, out_dim, 1, 1),
                                   nn.Conv2d(out_dim, out_channels, kernel_size=1, bias=False))

        if L_feature_v4:
            self.L_feature = BasicDCN_V4(c1_in_channels, out_dim, 3, 1, 1, offset_scale=L_feature_os)
        else:
...
if __name__ == '__main__':
    ...
    model = CVC_UnetrV4(in_channels=3, out_channels=2, ).to(device)
